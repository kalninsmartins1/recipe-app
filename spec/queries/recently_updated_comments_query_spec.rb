...
RSpec.describe RecentlyUpdatedCommentsQuery do
  ...
  it 'comments are ordered in descending order' do
    comment_a = create(:comment_a, recipe_id: recipe.id, chef_id: chef.id, created_at: 2.days.ago, updated_at: 2.days.ago)
    comment_b = create(:comment_b, recipe_id: recipe.id, chef_id: chef.id, created_at: 1.day.ago, updated_at: 2.hours.ago)
